...
public class OCLScopesGenitorTest extends AbstractTest {
        ...
        () -> {
          for (ASTOCLConstraint c : ast.get().getOCLArtifact().getOCLConstraintList()) {
            Assertions.assertNotNull(
                c.getEnclosingScope(),
                String.format(
                    "%s: The ast of the ocl constraint is missing its enclosing scope.",
  ...
  /**
   * Asserts that the enclosing scope of the ast of the ocl invariant is present and matches the
   * expected scope.
   *
  ...
  /**
   * Asserts that the spanned scope of the ast of the ocl invariant is present and linked with the
   * enclosing scope.
   *
   * @param inv the String assumed to be an ocl invariant
   */
  ...
  public void handleInvariant_shouldCreateSpannedScope(String inv) {
    ...
    Assertions.assertAll(
        () -> {
          ...
          Assertions.assertEquals(
              ast,
              ast.getSpannedScope().getAstNode(),
  ...
  /**
   * Asserts that the symbol of the ocl invariant is present and linked with its ast, spanned scope,
   * and enclosing scope.
   *
  ...
  public void handleInvariant_shouldCreateSymbol(String inv) {
    ...
    Assertions.assertAll(
        () -> {
          ...
          Assertions.assertEquals(
              ast.getSymbol(),
              ast.getSpannedScope().getSpanningSymbol(),
          ...
          Assertions.assertEquals(
              ast,
              ast.getSpannedScope().getSpanningSymbol().getAstNode(),
  ...
  /**
   * Parsers the provided string as ocl invariant, catching exceptions to the debug log and throwing
   * runtime errors in case of parser errors.
   *
   * @param inv the String to parse, assumed to be an ocl invariant, e.g., of the form `inv Name? (
  ...
  protected ASTOCLInvariant parseInvariant(String inv) {
    ...
    Preconditions.checkState(
        !OCLMill.parser().hasErrors(),
        String.format(
            "There where errors parsing the invariant `%s`. "
                + "The log lists the following findings: %s",
            inv, Log.getFindings()));
    Preconditions.checkState(
        ast.isPresent(),
        String.format(
            "The parser did not return an abstract syntax tree for invariant '%s'. "
                + "The log lists the following findings: %s",
            inv, Log.getFindings()));
    return ast.get();
  }
