...
public class VariableDeclarationOfCorrectType implements OCLExpressionsASTOCLVariableDeclarationCoCo {
  ...
  public void check(ASTOCLVariableDeclaration node) {
    if(!node.isPresentMCType()){
      Log.error(String.format("0xOCL30 Variable at %s has no type.", node.get_SourcePositionStart()));
    } else {
      TypeCheckResult result = getIDerive().deriveType(node.getExpression());
      if (!result.isPresentResult() || result.getResult().isObscureType()) {
        Log.error(String.format("0xOCL31 Type of Variable at %s could not be calculated.", node.get_SourcePositionStart()));
      }
      TypeCheckResult type = getISynthesize().synthesizeType(node.getMCType());
      if (!type.isPresentResult() || type.getResult().isObscureType()) {
        Log.error(String.format("0xOCL32 Type of Variable at %s could not be calculated.", node.get_SourcePositionStart()));
      }
      if (!OCLTypeCheck.compatible(result.getResult(), type.getResult())) {
        Log.error(String.format("0xOCL33 Type of Variable at %s is incompatible with given Declaration Type.",
