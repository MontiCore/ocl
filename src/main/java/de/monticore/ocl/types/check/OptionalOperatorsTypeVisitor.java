...
public class OptionalOperatorsTypeVisitor extends AbstractTypeVisitor
  ...
  public void endVisit(ASTOptionalLessEqualExpression expr) {
    ...
    SymTypeExpression result = TypeVisitorLifting
            .liftDefault((leftPar, rightPar) -> calculateTypeCompareOptional(expr.getLeft(), expr.getRight(), leftPar, rightPar))
            .apply(left, right);
    getType4Ast().setTypeOfExpression(expr, result);
  }
  ...
  public void endVisit(ASTOptionalEqualsExpression expr) {
    ...
    SymTypeExpression result = TypeVisitorLifting
            .liftDefault((leftPar, rightPar) -> calculateTypeLogicalOptional(expr.getLeft(), expr.getRight(), leftPar, rightPar))
            .apply(left, right);
    getType4Ast().setTypeOfExpression(expr, result);
  }
  ...
  public void endVisit(ASTOptionalSimilarExpression expr) {
    ...
    SymTypeExpression result = TypeVisitorLifting
            .liftDefault((leftPar, rightPar) -> calculateOptionalSimilarityExpression(expr, leftPar, rightPar))
            .apply(left, right);
    getType4Ast().setTypeOfExpression(expr, result);
  }
