...
under the name `MCOCL.jar` in your working directory:
```
...
In case you're using Docker, replace `java -jar MCOCL.jar` in the following 
...
gradle build
```
Congratulations! You can now find the executable JAR file `MCOCL.jar` in
...
that you locally named the tool `MCOCL`.
...
For trying this out, copy the `MCOCL.jar` into a directory of your 
choice. 
...
Save the text file as `Example.ocl` in the directory where `MCOCL.jar` is 
located. 
...
save it in a file `Bookshop.ocl` in the directory where the file `MCOCL.jar` 
is located. 
...
If you don't want to get involved with the CD tool at this point, you can also find the 
