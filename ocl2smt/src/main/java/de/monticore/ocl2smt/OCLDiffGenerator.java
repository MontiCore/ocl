...
public class OCLDiffGenerator {
  ...
  protected static List<IdentifiableBoolExpr> getPositiveSolverConstraints(
      ASTCDCompilationUnit astcd, Set<ASTOCLCompilationUnit> in) {
  ...
  public static Pair<ASTODArtifact, Set<ASTODArtifact>> oclDiff(
    ...
    notIn.forEach(
        p ->
            ocl2SMTGenerator
                .ocl2smt(p.getOCLArtifact())
                .forEach(
                    idf ->
                        negConstList.add(
                            idf.negate(ocl2SMTGenerator.cd2smtGenerator.getContext()))));
    ...
    return new ImmutablePair<>(
        TraceUnsatCore.buildUnsatOD(solverConstraints, negConstList, traceUnsat), satOdList);
  }
